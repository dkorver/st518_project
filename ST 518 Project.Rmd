...
However, because this is a mixed model, we must consider the Type III SS analyses, which are calculated below...  

The following results display the F and p values for the main effect `Brand`. We can see that this effect is *not* significant at the 5% level.
```{r brand}
F.brand <- MS.brand/MS.brand.temp
F.brand
p.brand <- pf(F.brand,1,2,lower.tail=F)
p.brand
```

The following results display the F and p values for the main effect `Stirred`. We can see that this effect is significant at the 5% level.
```{r stirred}
F.stirred <- MS.stirred/MS.temp.stirred
F.stirred
p.stirred <- pf(F.stirred,1,2,lower.tail=F)
p.stirred
```

The following results display the F and p values for the interaction effect `Brand:Temp`. We can see that this effect is significant at the 5% level.
```{r brand.temp}
F.brand.temp <- MS.brand.temp/MS.brand.temp.stirred
F.brand.temp
p.brand.temp <- pf(F.brand.temp,2,2,lower.tail=F)
p.brand.temp
```

The following results display the F and p values for the interaction effect `Brand:Stirred`. We can see that this effect is *not* significant at the 5% level.
```{r brand.stirred}
F.brand.stirred <- MS.brand.stirred/MS.brand.temp.stirred
F.brand.stirred
p.brand.stirred <- pf(F.brand.stirred,1,2,lower.tail=F)
p.brand.stirred
```

The following results display the F and p values for the interaction effect `Temp:Stirred`. We can see that this effect is *not* significant at the 5% level.
```{r}
F.temp.stirred <- MS.temp.stirred/MS.brand.temp.stirred
F.temp.stirred
p.temp.stirred <- pf(F.temp.stirred,2,2,lower.tail=F)
p.temp.stirred
```

The following results display the F and p values for the interaction effect `Brand:Temp:Stirred`. We can see that this effect is significant at the 5% level.
```{r brand.temp.stirred}
F.brand.temp.stirred <- MS.brand.temp.stirred/MSE
F.brand.temp.stirred
p.brand.temp.stirred <- pf(F.brand.temp.stirred,2,36,lower.tail=F)
p.brand.temp.stirred
```

In reviewing the analysis of variance table for this model, we can see that all model terms are significant at the 5% level aside from one, the interaction effect between `Temperature` and `Stirred`. This is confirmed in the interaction plot below, where we observe the two lines to be parallel. We can see that the temperature reduces the dissolve time and whether or not it is stirred also affects dissolve time, but that temperature of the water and stirred status do not depend on the value of one another.
