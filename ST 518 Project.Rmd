...
Our model is $Y_{ijkl}=\mu+\alpha_i+\beta_j+\gamma_k+(\alpha\beta)_{ij}+(\alpha\gamma)_{ik}+(\beta\gamma)_{jk}+(\alpha\beta\gamma)_{ijk}+E_{ijkl}$  

for $i=1,2$;  $j=1,2,3$;  $k=1,2,3,4$;  $l=1,...,48$   
...
$\beta\gamma_{jk}$ is the interaction effect between the jth temp and kth stirred status  
$E_{ijkl}$ is the error term  
